...
[<img class="aligncenter size-full wp-image-290" alt="Untitled1" src="/images/posts/2013/05/Untitled11.png" width="652" height="345" srcset="/images/posts/2013/05/Untitled11.png 652w, /images/posts/2013/05/Untitled11-300x159.png 300w, /images/posts/2013/05/Untitled11-310x165.png 310w" sizes="(max-width: 652px) 100vw, 652px" />](/images/posts/2013/05/Untitled11.png)

The next factor that has to be considered is the size of the timer / counter module. This will define the maximum number of counts the module can make before overflowing. Essentially, a 8-bit timer module can hold only the record of 256 clock pulses or external events before it overflows or triggers and interrupt. So the more the bits, the higher time the timer can last before overflowing. Timers may be up-counters (counting up from 0 to N) or down-counter (counting down from N to 0)A and depends on the architecture of each microcontroller.
...
## **Understanding the****A concept of** prescaler and postscaler:

The prescaler and postscaler are fairly common in most of the mid-range and advanced microcontrollers. They are provided as added feature and may or may not be used by the programmer. In a way they are one and the same. They both are used to control the overflow rate the module.

[<img class="aligncenter size-full wp-image-1300" alt="Timer scaling ratio" src="/images/posts/2013/05/Untitled2.png" width="549" height="209" srcset="/images/posts/2013/05/Untitled2.png 549w, /images/posts/2013/05/Untitled2-300x114.png 300w" sizes="(max-width: 549px) 100vw, 549px" />](/images/posts/2013/05/Untitled2.png)The the value in the prescaler register defines the number of times the clock has to tick before the timer register experiences a single pulse. This can be visualized from the above image. The scaling ratio 1:2 means that the clock has to have aA LOW to HIGH transition twice so that the output of the prescaler completes one HIGH pulse. This is one way of reducing the clock frequency. Hence by using this method, the timer register is essentially multiplied by 'N', where N is the value of the prescaler.
...
In case of up counter,
  
C = N x [timer-register-value] x M
  
In case of down counter,
  
C = N x [(timer-register-value)-(maximum-value-timer-register)] x M

Having said this, you are now able to calculate the number of times the timer will count before it will interrupt the CPU. With this data at your disposal and the system clock frequency you can program the tier module to produce accurate time intervals.

## **Calculating the time interval for a given C value:**

Frequency is basically, number of times something happens in one second. Tell that to any one and be sure you said it right dYtm, So a 20MHz of clock frequency is actually, 20 x 10<sup>6</sup> clock cycles in one second. To obtain the time taken for one clock cycle you will have to take the inverse of the frequency. In this case it is 1/20MHz which will give me, 0.05x10<sup>-</sup><sup>6</sup> seconds.

But not all the controller take the machine clock as it is, it is scaled down by a factor, which varies from one controller to another, so this part is not controller independent as it was meant to be, You will have to refer with you datasheet to find out your scaling factor.
...
## **Calculating the C****A value** for a given timer interval:
...
It's as simple as that. Now all you have to do is properly divide the count value into the prescaler, postscaler, and timer register. Once all the registers are set and the suitable interrupt setting are made, the controller will interrupt the CPU once every second and jump into the Interrupt Service Routine (ISR).
...
I hope this was to an extent helpful in understanding timer / counter modules and their interface. If you have any doubts or queries regarding this post please let me know and I will try to answer them to the best of my abilities.

If you found this post helpful, <a href="http://embedjournal.com/subscribe/" target="_blank">subscribe to our newsletters</a> with you email using the form below and we will keep you posted on the latest updates of this blog.